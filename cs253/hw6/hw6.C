...
{
  // Use BFS to find a shortest path from s to t, and record the
  // path on the given List<int> (provided by the caller).
  // Returns non-zero iff a path was found.
...
public:
  ...
  Symbol<Name,Attribute>* SymFind(const Name& name) {
    ...
    element=name.Hash(name, buckets);
...
const int MaxGraphSize = 7000;  // upper bound on number of vertices
