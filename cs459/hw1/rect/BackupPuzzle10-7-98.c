...
char **argv;
{
	...
	while (1)  {
		...
		case ButtonPress:
			...
			if (ycoord<=30)  printf("click observed in upper rectangle\n");
			...
			/* if ((dist > 0) && (dirv==1))  then tile above
			   temp1=tile[xempty][(yempty-dist)].index;
			if ((dist < 0) && (dirv==1))  then tile below
			   
			if ((dist > 0) && (dirv==0))  then tile to left
			   
			if ((dist < 0) && (dirv==0))  then tile to right */
